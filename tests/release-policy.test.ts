...
describe('static release policy', () => {
  ...
  it('keeps review-required labels concrete and removes public asset claims', () => {
    ...
    expect(home).toContain('Repeatable seed');
    expect(home).toContain('<strong>Change</strong>');
    expect(home).not.toContain('inspectable numbers');
    expect(home).not.toContain('Deterministic seed');
    ...
    expect(readme).toContain('predict, change, inspect, and explain');
    expect(readme).toContain('Three models cover nearest-neighbor routes');
    expect(readme).not.toContain('bounded templates');
    expect(readme).not.toContain('Requires Node.js');
    expect(readme).not.toContain('Playwright is pinned');
